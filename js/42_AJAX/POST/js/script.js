...
document.addEventListener('DOMContentLoaded', function () {
	...
	*/
	...
	modalWindow.addEventListener('click', function (event) {
		if (event.target == modalWindow || event.target.getAttribute('data-close-modal') === '') {
			closeModal()

		} /* Таким образом мы сделали что при клике на подложку то есть затемненную часть за модальным окном, ИЛИ если у элемента есть крестик, то есть атрибут data-close-modal, модалка закрывается. То есть необязательно жать крестик для закрытия. Для этого ставим условием что событие клика произошло конкретно на подложке (в которой и есть сама модалка) то все закрывается и страница листается как обычно */
	})
	...
	*/
	...
	const forms = document.querySelectorAll('form'); // Получаем со страницы все формы с тегом "form"
	...
	forms.forEach(function (item) {
		postData(item)
	}); /* Перебираем все формы на странице и каждую из форм вставляем в параметры функции postData  */


	function postData(form) { // функция будет отвечать за постинг данных. В параметры функции принимаем какую-то форму которую мы передадим
		form.addEventListener('submit', function (event) { /* Событие submit происходит когда мы отправляем форму  */
			...
			const request = new XMLHttpRequest(); /* Создаем объект request */

			request.open('POST', 'server.php') // передаем метод передачи, и тот путь на который будет ссылаться


			/* 	request.setRequestHeader('Content-type', 'multipart/form-data');
				Указываем заголовки, тип контента multipart/form-data, это надо чтобы работать с объектов form-data 
	
					(ВНИМАНИЕ!!! ПРИ СВЯЗКЕ объекта XMLHttpRequest() и объекта FormData заголовок устанавливать не нужно, он устанавливается автоматически) */
			...
			request.send(formData); /* Отправляем сформированный объект formData */
			...
			request.addEventListener('load', function () {
				if (request.status === 200) {
					...
					const formInputs = form.querySelectorAll('input'); /* Берем все инпуты из формы */
					formInputs.forEach(function (input) {
						input.value = ''
					}) /* Удаляем все введенную информацию из инпутов после успешной отправки данных */

					/* form.reset(); Можно написать и так, работать будет также */
	...
	function showThanksModal(message) { /* Создаем скрипт для показа окна благодарности после отправки формы */
		...
		thanksModal.innerHTML = `
			<div class='modal__content'>
				<div class="modal__close" data-close-modal>&times;</div>
