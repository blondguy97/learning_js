...
2) Второе это написать в js элемент (который мы нашли через DOM и присвоили переменной), и также через on добавить событие и просвоить этому какую то функцию где и прописать что нужно сделать, или же присвоить уже готовую функцию которая была создана раннее
    ...
    Здесь после выбранного элемента идем метод addEventListener, после чего в скобках пишем само действие которые будет выполнено, в этот раз без приставки on, а затем идет callback функция (можно использовать как обычную, так и стрелочную), внутри которой будут идти действия после совершения пользователем нажатия клавиши или другого события
...
r_square.addEventListener('click', delElement); // Замечу что мы НЕ вызываем функцию, после события клика, то есть все без круглых скобок, мы лишь ссыламся на функцию delElement
...
function showSomeInfo(event) {
    console.log(event.target);
    i++;
    if (i == 3) {
        p_square.removeEventListener('click', showSomeInfo);
    }
}

p_square.addEventListener('click', showSomeInfo);
...
/* таким образом для пурпурного квадрата мы создали событие клика при котором в консоль выводится ссылка на этот элемент, и через условие что клик был всего раз сделан, событие удаляется через обработчик событий removeEventListener. И все это мы передали через функцию showSomeInfo. Так мы сказали что событие клика должно происходить всего 1 раз
...
function showSomeInfo_2(event) {
    console.log(event.target);
}


p_square.addEventListener('click', showSomeInfo_2, { once: true });
...
1) Устаревший вариант, в конце функции обработчика событий return false. Но этим никто уже не пользуется. К тому же надо использовать при помощи устаревшего onClick обработчика
